...
public class CadastroEventoFrame extends JFrame {
    ...
    // --- Constantes de Estilo (Replicadas para exemplo, idealmente em uma classe de utilidades) ---
    private static final Color COR_FUNDO_PAINEL = new Color(250, 250, 250); // Usado para o painel principal
    private static final Color COR_TITULO_FRAME = new Color(60, 70, 90); // Não usado diretamente no título do frame, mas poderia ser
    ...
    public CadastroEventoFrame(Evento evento, ConsultaEventosFrame consultaFramePai) {
        ...
        // getContentPane().setBackground(new Color(220,220,220)); // Cinza claro para fundo do frame
    ...
    private void executarAcaoPrincipal() {
        ...
        if (dataStr.contains("_") || dataStr.equals("//")) { // Checa placeholder ou campo vazio pela máscara
            JOptionPane.showMessageDialog(this, "A data está incompleta ou vazia. Use o formato DD/MM/AAAA.", "Data Inválida", JOptionPane.ERROR_MESSAGE);
    ...
    public void limparEValidarDataField(String dataStr) {
        if (dataStr == null || dataStr.trim().isEmpty() || dataStr.equals("//")) { // Adicionado "//" para caso de campo vazio com máscara
