...
public class ConsultaFrame extends JFrame {
  ...
  private void atualizarTabela() {
    ...
    for (int i = 0; i < participantes.size(); i++) {
      ...
      dados[i][8] = p.getEvento() != null ? p.getEvento() : "N/A";
    }
  ...
  private void editarParticipante() {
    ...
    if (linha >= 0) {
      ...
      JTextField eventoField = new JTextField(p.getEvento());
      ...
      salvarButton.addActionListener(e -> {
        ...
        if (nome.isEmpty() || sexo.isEmpty() || email.isEmpty() || celular.isEmpty() || evento.isEmpty()) {
        ...
        dao.atualizar(p); // Atualizando no banco de dados
        JOptionPane.showMessageDialog(editarFrame, "Participante atualizado com sucesso!");
        editarFrame.dispose();
        dispose();
        new ConsultaFrame(); // Atualiza a lista de participantes
      });
