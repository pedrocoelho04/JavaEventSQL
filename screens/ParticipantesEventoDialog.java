...
public class ParticipantesEventoDialog extends JDialog {
    ...
    public ParticipantesEventoDialog(Frame owner, int eventoId, String eventoTitulo) {
        ...
        // this.eventoTitulo = eventoTitulo; // Armazenado se necessário para outros usos
    ...
    private void carregarTodosParticipantesCache() {
        ...
        if (todosParticipantesCache == null) {
            ...
            JOptionPane.showMessageDialog(this, "Não foi possível carregar a lista de todos os participantes.", "Erro de Dados", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void carregarParticipantesInscritos() {
        ...
        if (inscricoes != null) {
            for (EventoParticipante ep : inscricoes) {
             ...
             JOptionPane.showMessageDialog(this, "Não foi possível carregar os participantes inscritos.", "Erro de Dados", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void carregarOutrosParticipantes(String criterioBusca, String valorBuscaStr) {
        ...
        // Alternativamente, buscar novamente do DAO se preferir garantir dados mais recentes
        // List<EventoParticipante> inscricoes = eventoParticipanteDao.listarPorParametro(this.eventoId, null);
        // if (inscricoes != null) {
        //     for (EventoParticipante ep : inscricoes) {
        ...
        if ("Todos".equals(criterioBusca) || valorBuscaFinal.isEmpty() && !"ID".equals(criterioBusca)) {
            // Se for "Todos", ou se o valor de busca estiver vazio (exceto para ID, onde vazio não faz sentido)
             resultadoFiltragem.addAll(participantesParaConsiderar);
        } else {
            switch (criterioBusca) {
                case "ID":
                    if (valorBuscaFinal.isEmpty()) {
                        JOptionPane.showMessageDialog(this, "Por favor, insira um ID para buscar.", "ID Vazio", JOptionPane.WARNING_MESSAGE);
                    ...
                    try {
                        ...
                        JOptionPane.showMessageDialog(this, "ID inválido. Deve ser um número.", "Erro de Formato", JOptionPane.ERROR_MESSAGE);
        ...
        // Adiciona à tabela 'modelOutros' apenas os que estão em 'resultadoFiltragem' E não estão em 'idsInscritos'
    ...
    private void adicionarParticipanteAoEvento() {
        ...
        if (linhaSelecionada >= 0) {
            try {
                ...
                // List<EventoParticipante> inscricoes = eventoParticipanteDao.listarPorParametro(this.eventoId, idParticipante);
                // if (inscricoes != null && !inscricoes.isEmpty()){
                //     jaInscrito = true;
                // }

                if(jaInscrito){
                     JOptionPane.showMessageDialog(this, "Este participante já está inscrito neste evento.", "Atenção", JOptionPane.WARNING_MESSAGE);
                     return;
                }
                ...
                if ("sucesso".equals(resultado)) {
                    ...
                    // Atualizar cache ou refazer chamadas DAO seria mais robusto se o cache não for alterado aqui
                    carregarParticipantesInscritos(); 
                    carregarOutrosParticipantes((String) comboBoxCriterioBusca.getSelectedItem(), campoValorBusca.getText());
                } else {
                    JOptionPane.showMessageDialog(this, "Erro ao adicionar participante: " + resultado, "Erro", JOptionPane.ERROR_MESSAGE);
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Erro ao processar adição: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
        } else {
            JOptionPane.showMessageDialog(this, "Selecione um participante na tabela 'Outros Participantes' para adicionar.", "Nenhum Participante Selecionado", JOptionPane.INFORMATION_MESSAGE);
        }
    }
            ...
            try {
                ...
                if (relacoes != null && !relacoes.isEmpty()) {
                    ...
                    if ("sucesso".equals(resultado)) {
                        ...
                        JOptionPane.showMessageDialog(this, "Erro ao remover participante: " + resultado, "Erro", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                     JOptionPane.showMessageDialog(this, "Não foi possível encontrar a inscrição do participante para remover.", "Erro de Dados", JOptionPane.ERROR_MESSAGE);
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Erro ao processar remoção: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
        } else {
            JOptionPane.showMessageDialog(this, "Selecione um participante na tabela 'Participantes Inscritos' para remover.", "Nenhum Participante Selecionado", JOptionPane.INFORMATION_MESSAGE);
        }
    }
}